...
def json_response(handler: SimpleHTTPRequestHandler, status: int, payload: dict):
...
def image_response(handler: SimpleHTTPRequestHandler, status: int, data: bytes, content_type: str = "image/jpeg"):
    handler.send_response(status)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/place-photo/img":
            result = serve_place_image(params)
            if not result:
                json_response(self, 404, {"error": True, "message": "이미지를 찾지 못했습니다."})
                return
            data, content_type = result
            image_response(self, 200, data, content_type)
            return
        ...
        if parsed.path == "/api/place-photo":
            ...
            try:
                ...
                lat = lng = None
            if not name:
                json_response(self, 400, {"error": True, "message": "name 또는 q 파라미터가 필요합니다."})
                return
            src = (params.get("src") or [""])[0].strip()
            ...
            if not remote:
                json_response(self, 404, {"error": True, "name": name, "url": ""})
                return
            json_response(self, 200, {"name": name, "url": proxy, "remote": remote})
            return

        if parsed.path == "/api/transit-search":
            q = (params.get("q") or [""])[0]
            limit = min(int((params.get("limit") or ["8"])[0]), 15)
            items = search_naver_places(q, limit)
            json_response(self, 200, {"query": q, "items": items})
            return

        if parsed.path == "/api/nearby-places-all":
            try:
                lat = float((params.get("lat") or ["0"])[0])
                lng = float((params.get("lng") or ["0"])[0])
            except ValueError:
                json_response(self, 400, {"error": True, "message": "좌표 형식이 올바르지 않습니다."})
                return
            try:
            ...
            if not (124 <= lng <= 132 and 33 <= lat <= 39):
                json_response(self, 400, {"error": True, "message": "한국 내 좌표만 지원합니다."})
                return
            payload = {
                "lat": lat,
                "lng": lng,
        ...
        if parsed.path == "/api/nearby-places":
            try:
                lat = float((params.get("lat") or ["0"])[0])
                lng = float((params.get("lng") or ["0"])[0])
            except ValueError:
            ...
            json_response(self, 200, {"lat": lat, "lng": lng, "kind": kind, "radiusKm": radius, "items": items})
            return
        ...
        if parsed.path == "/api/geocode":
            ...
            if not point:
                json_response(self, 404, {"error": True, "message": "위치를 찾지 못했습니다.", "query": q})
                return
            json_response(self, 200, {"query": q, "point": point})
            return

        if parsed.path == "/api/kric/station-mobility":
            result = get_station_mobility(params)
            if result.get("error") and not result.get("fallback"):
                json_response(self, 503, result)
                return
            json_response(self, 200, result)
            return

        if parsed.path == "/api/transit-route":
...
def main():
    ...
    print("Place photo:  /api/place-photo?name=&address=")
    print("Nearby:       /api/nearby-places?lat=&lng=&kind=cafe|food|shop&radius=5")
    print("Routes:       /api/transit-route?fromLat=&fromLng=&toLat=&toLng=&mode=transit")
    print("KRIC mobility:/api/kric/station-mobility?rail=&ln=&stin=&name=...")
    if key:
        print("ODsay API key: loaded")
    else:
        print("ODsay API key: not set (transit mode needs odsay-config.local.json)")
    if kric:
        print("KRIC API key: loaded")
    else:
        print("KRIC API key: not set (역사 교통약자 안내는 샘플·제한 데이터)")
    try:
